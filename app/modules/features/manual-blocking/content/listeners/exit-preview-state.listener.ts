...
@injectable()
export class ExitPreviewStateListener implements AppMessageListener<ManualBlockingExitPreviewMessage> {
  constructor (
    @inject(ContentManualBlockingIdentifiers.service)
    private readonly selector: SelectorService
  ) {}

  on (): ManualBlockingMessages.exitPreview {
    return ManualBlockingMessages.exitPreview
  }
